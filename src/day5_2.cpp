...
public:
    ...
    void addCommand(Command &cmd)
    {
        ...
        switch (cmd.getDirection())
        {
            ...
            case DIRECTION_DIAGNAL:
                if (cmd.getDistanceX() > 0 && cmd.getDistanceY() > 0)
                {
                    for (int i = x; i <= x + cmd.getDistanceX(); i++)
                    {
                        field[y++][i]++;
                    }
                
                }
                else if (cmd.getDistanceX() < 0 && cmd.getDistanceY() > 0)
                {
                    for (int i = x; i >= x + cmd.getDistanceX(); i--)
                    {
                        field[y++][i]++;
